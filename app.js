document.addEventListener('DOMContentLoaded', () => {
   ...
   // EVENTO COMPARTIDO PARA EL BOTÓN DE BORRAR (CORREGIDO)
    document.addEventListener('click', (e) => {
        // Buscamos si el elemento pulsado es el botón o está dentro de él
        ...
        if (botonBorrar) {
            ...
            if (confirm('¿Estás seguro de que deseas borrar por completo este registro semanal? Esta acción afectará también al usuario.')) {
                // 1. Filtramos los datos para eliminar el registro coincidente
                sistemaDatos = sistemaDatos.filter(registro => String(registro.id) !== String(idParaBorrar));
                
                // 2. Guardamos los cambios en la memoria del navegador
                guardarEnMemoria();
                
                // 3. Volvemos a pintar la tabla del gestor totalmente limpia
                renderManagerTable();
