document.addEventListener('DOMContentLoaded', () => {
    ...
    // CONTENEDORES DINÁMICOS DE RENDIMIENTO
    ...
    // --- EVALUACIÓN DE INTERFAZ SEGÚN USUARIO ACTIVO ---
    function evaluarVisibilidadModulos() {
        if (currentUser && currentUser.endsWith('@acerca.info')) {
            // El acceso con correo personal corporativo solo permite rellenar datos de sala
            moduloRrhhContainer.style.display = 'none';
            wrapperGrid.classList.remove('double');
        } else {
            // Usuarios administradores o especiales ven la interfaz completa
            moduloRrhhContainer.style.display = 'block';
            wrapperGrid.classList.add('double');
        }
    }
    ...
    // --- ACCESO CON CONTROL DE VISIBILIDAD ---
    ...
    // --- MÓDULO 3: PANEL DEL SUPERVISOR GESTOR ---
    function renderSupervisorScreen() {
        ...
        if(!baseDatosLotes.length) managerGlobalTableBody.innerHTML = `<tr><td colspan="6" style="text-align:center;color:#999;">Sin lotes grabados.</td></tr>`;
        else {
            baseDatosLotes.forEach(l => {
                const tr = document.createElement('tr');
                tr.innerHTML = `<td>${l.responsable}</td><td><b>Semana ${l.semana}</b></td><td>${l.ciudad}</td><td>${l.departamento}</td><td><b>${l.candidatos.length}</b> altas</td><td><button class="btn-primary rev-btn" style="padding:4px 8px; font-size:11px;">Revisar</button><button class="del-lote-btn" style="background:none;border:none;cursor:pointer;margin-left:5px;">🗑️</button></td>`;
                tr.querySelector('.rev-btn').addEventListener('click', () => verDetalleDeLote(l.id));
                tr.querySelector('.del-lote-btn').addEventListener('click', () => { if(confirm('¿Borrar lote completo?')) { baseDatosLotes = baseDatosLotes.filter(bl => bl.id !== l.id); localStorage.setItem('acerca_lotes_erp', JSON.stringify(baseDatosLotes)); renderSupervisorScreen(); } });
                managerGlobalTableBody.appendChild(tr);
            });
        }
    }

    // --- ESCUCHADORES DE MODALES EXTRAS ---
    ...
    exportExcelBtn.addEventListener('click', () => {
        let rows = [];
        baseDatosLotes.forEach(l => l.candidatos.forEach(c => rows.push({ Responsable: l.responsable, Semana: l.semana, Sede: l.ciudad, Departamento: l.departamento, Candidato: c.nombre, DNI: c.dni, Teléfono: c.telefono, Email: c.email, Turno: c.turno, Jornada: c.jornada, Observaciones: c.observaciones })));
        ...
        XLSX.writeFile(libro, "ERP_Acerca_Completo.xlsx");
    });
